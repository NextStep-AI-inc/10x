...
| 9. Release-Build End-to-End Acceptance | recorded with one external blocker | `09a3215` | `0b8ad26` | Final review approved | Exact-head gates pass: OmpKit 165 with 2 expected skips, app tests 223, universal Release build, strict local signature validation, bundled Hammerspoon contract inspection, and visible packaged Release PID 25273. Configured-desktop acceptance installed and exercised AeroSpace and Hammerspoon. Direct AeroSpace isolation preserved focus, pointer, current workspace/Space, and existing windows. Hammerspoon private Spaces calls were no-ops and now fail closed through user-space and postcondition checks. Setup now waits for exact probe-window registration. Full OMP task evidence remains blocked by macOS Accessibility identity for the ad-hoc local worker; Background Only was not run. |

## Rulings and follow-ups

- No deployment, merge, or publication is authorized by this execution. With explicit user authorization for machine setup, AeroSpace `0.21.3-Beta` and Hammerspoon `1.1.1` were installed and configured for physical acceptance.
...
- Task 9: Ruling: direct helper evidence is separated from the blocked full 10x + OMP flow. AeroSpace isolation passed physically. Hammerspoon's current macOS private Spaces calls were no-ops and the bridge now detects and fails them closed. Older OMP 18.0.4 gating passed in the real Release UI. The full flow cannot be called release-ready until a signed/responsible OMP worker obtains macOS Accessibility and Background Only is exercised.
